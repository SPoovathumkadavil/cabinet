...
#let margincite(key, mode, prefix, suffix, noteNum, hash) = context {
  if query(bibliography).len() > 0 {
    ...
    if supplement != none and supplement.len() > 0 { cite(key, form: "normal", supplement: supplement) } else {
    ...
    [#margin-note(dy: dy, dx: .25in)[
        #if supplement != none and supplement.len() > 0 { cite(key, form: "full", supplement: supplement) } else {
          cite(key, form: "full")
        }]
    ]
  }
...
#let article(
  ...
  // Page setup
  let lr(l, r, ..kwargs) = wideblock(..kwargs, grid(
    columns: (1fr, 4fr),
    align(left, text(size: 8pt, fill: gray, l)), align(right, text(size: 8pt, fill: gray, r)),
  ))
  set page(
    paper: "us-letter",
    ...
    header: context {
      if counter(page).get().first() > 1 {
        ...
        lr([], [#if shorttitle != none { upper(shorttitle) } else { upper(title) }
  ...
  set text(fill: luma(30), lang: lang, region: region, historical-ligatures: true)
  ...
  show raw: set text(font: "Liga SFMono Nerd Font", size: 8pt, ligatures: false)
